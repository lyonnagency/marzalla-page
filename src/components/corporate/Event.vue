<template lang="pug">
  .modal(@click.stop.prevent="close").corporate
    article
      .container(@click.stop="")
        ...
        header
          h2 {{ event.name }}
        .image(:style="'background-image: url(/static/img/corporate/events/' + event.image + ')'")
        .description
          section
            .information
              div
...
export default {
  ...
  beforeMount() {
    ...
    this.$store.dispatch('getEvent', { area: "corporate", code_name: this.code_name })
  },
  computed: mapGetters([
    'event'
  ]),
